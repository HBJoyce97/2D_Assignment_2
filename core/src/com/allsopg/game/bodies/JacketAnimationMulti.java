...
public class JacketAnimationMulti extends Sprite {

    public Animation idle, jacket, animation; // Variables used to set the current animation
    ...
    public JacketAnimationMulti(String atlasString_1, String atlasString_2, Texture t, Vector2 pos, Animation.PlayMode loopType){
        super(t, 2, 2);
        ...
        regions_1.sort(new JacketAnimationMulti.RegionComparator());
        idle = new Animation(Constants.JACKET_FRAME_DURATION, regions_1, loopType);
        ...
        regions_2.sort(new JacketAnimationMulti.RegionComparator());
        jacket = new Animation(Constants.JACKET_FRAME_DURATION, regions_2, loopType);
        ...
        Tween.to(tweenData, TweenDataAccessor.TYPE_POS,0f) // Sets the start position of the sprite (off screen in the center)
                ...
                .setCallback(new TweenCallback() { // Initializes the callback
                    ...
                    public void onEvent(int type, BaseTween<?> source) { // The following event causes the animation change
                        soundLink.play(3); // Play the third sound in the IntMap (Armour)
                        animation = jacket; // Change the animation from 'idle' to 'jacket'
                    }
                }).start(tweenManager)
